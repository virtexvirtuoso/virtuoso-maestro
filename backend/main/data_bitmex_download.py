...
if __name__ == '__main__':
    ...
    bitmex_config = config_reader.get_datasource_config(provider=DataSourceProviders.BITMEX)
    bitmex_downloader = BitMexBatchDownloader(storage=storage)

    for symbol, sconf in bitmex_config.symbols.items():
        for bin_size in sconf.bin_size:
            bitmex_downloader.register(symbol=symbol, bin_size=bin_size)

    bitmex_downloader.start()
